...
NumeralFormatter.prototype = {
  ...
  format: function (value) {
    const owner = this; let parts; let partSign; let partSignAndPrefix; let partInteger; let partDecimal = ''
...
const NumeralFormatter_1 = NumeralFormatter

const DateFormatter = function (datePattern, dateMin, dateMax) {
  const owner = this
  owner.date = []
  owner.blocks = []
...
DateFormatter.prototype = {
  ...
  getISOFormatDate: function () {
    const owner = this; const date = owner.date
  ...
  getValidatedDate: function (value) {
    const owner = this; let result = ''
    value = value.replace(/[^\d]/g, '')
    owner.blocks.forEach(function (length, index) {
      if (value.length > 0) {
        let sub = value.slice(0, length); const sub0 = sub.slice(0, 1); const rest = value.slice(length)
  ...
  getFixedDateString: function (value) {
    const owner = this; const datePattern = owner.datePattern; let date = []; let dayIndex = 0; let monthIndex = 0; let yearIndex = 0; let dayStartIndex = 0; let monthStartIndex = 0; let yearStartIndex = 0; let day; let month; let year; let fullYearDone = false
  ...
  getRangeFixedDate: function (date) {
    const owner = this; const datePattern = owner.datePattern; const dateMin = owner.dateMin || []; const dateMax = owner.dateMax || []
...
const DateFormatter_1 = DateFormatter

const TimeFormatter = function (timePattern, timeFormat) {
  const owner = this
  owner.time = []
  owner.blocks = []
...
TimeFormatter.prototype = {
  ...
  getISOFormatTime: function () {
    const owner = this; const time = owner.time
  ...
  getValidatedTime: function (value) {
    const owner = this; let result = ''
    value = value.replace(/[^\d]/g, '')
    const timeFormatOptions = owner.getTimeFormatOptions()
    owner.blocks.forEach(function (length, index) {
      if (value.length > 0) {
        let sub = value.slice(0, length); const sub0 = sub.slice(0, 1); const rest = value.slice(length)
  ...
  getFixedTimeString: function (value) {
    const owner = this; const timePattern = owner.timePattern; let time = []; let secondIndex = 0; let minuteIndex = 0; let hourIndex = 0; let secondStartIndex = 0; let minuteStartIndex = 0; let hourStartIndex = 0; let second; let minute; let hour
    if (value.length === 6) {
...
const TimeFormatter_1 = TimeFormatter

const PhoneFormatter = function (formatter, delimiter) {
  const owner = this
  owner.delimiter = delimiter || delimiter === '' ? delimiter : ' '
  owner.delimiterRE = delimiter ? new RegExp('\\' + delimiter, 'g') : ''
...
PhoneFormatter.prototype = {
  ...
  format: function (phoneNumber) {
    ...
    let result = ''; let current; let validated = false
...
const PhoneFormatter_1 = PhoneFormatter
...
var CreditCardDetector = {
  ...
  getInfo: function (value, strictMode) {
    const blocks = CreditCardDetector.blocks; const re = CreditCardDetector.re
...
const CreditCardDetector_1 = CreditCardDetector
...
const Util = {
  ...
  getPrefixStrippedValue: function (value, prefix, prefixLength, prevResult, delimiter, delimiters, noImmediatePrefix, tailPrefix, signBeforePrefix) {
    ...
    if (signBeforePrefix && value.slice(0, 1) == '-') {
      const prev = prevResult.slice(0, 1) == '-' ? prevResult.slice(1) : prevResult
  ...
  getFormattedValue: function (value, blocks, blocksLength, delimiter, delimiters, delimiterLazyShow) {
    let result = ''; const multipleDelimiters = delimiters.length > 0; let currentDelimiter = ''
    if (blocksLength === 0) {
      return value
    }
    blocks.forEach(function (length, index) {
      if (value.length > 0) {
        const sub = value.slice(0, length); const rest = value.slice(length)
  ...
  fixPrefixCursor: function (el, prefix, delimiter, delimiters) {
    ...
    const val = el.value; const appendix = delimiter || (delimiters[0] || ' ')
...
const DefaultProperties_1 = DefaultProperties
...
Cleave.prototype = {
  ...
  initSwapHiddenInput: function () {
    ...
    const inputFormatter = owner.element.cloneNode(true)
  ...
  onKeyDown: function (event) {
    const owner = this; const charCode = event.which || event.keyCode
    owner.lastInputValue = owner.element.value
    owner.isBackward = charCode === 8
  },
  onChange: function (event) {
    const owner = this; const pps = owner.properties; const Util = Cleave.Util
    owner.isBackward = owner.isBackward || event.inputType === 'deleteContentBackward'
    const postDelimiter = Util.getPostDelimiter(owner.lastInputValue, pps.delimiter, pps.delimiters)
  ...
  copyClipboardData: function (e) {
    const owner = this; const pps = owner.properties; const Util = Cleave.Util; const inputValue = owner.element.value; let textToCopy = ''
  ...
  onInput: function (value) {
    const owner = this; const pps = owner.properties; const Util = Cleave.Util
    const postDelimiterAfter = Util.getPostDelimiter(value, pps.delimiter, pps.delimiters)
  ...
  updateCreditCardPropsByValue: function (value) {
    const owner = this; const pps = owner.properties; const Util = Cleave.Util; let creditCardInfo
  ...
  updateValueState: function () {
    const owner = this; const Util = Cleave.Util; const pps = owner.properties
    if (!owner.element) {
      return
    }
  ...
  getRawValue: function () {
    const owner = this; const pps = owner.properties; const Util = Cleave.Util; let rawValue = owner.element.value
